...
import { type TraceLabels, type TraceQuestionId, type TraceTriageInput, traceQuestions } from './trace.js';
...
export class MockProvider implements DecisionProvider {
  ...
  async evaluateTrace(input: TraceTriageInput): Promise<TraceLabels> {
    ...
    return { task_outcome:mk(Object.keys(traceQuestions.task_outcome),outcome), user_outcome:mk(Object.keys(traceQuestions.user_outcome),user), run_health:mk(Object.keys(traceQuestions.run_health),health) } as TraceLabels;
  }
}
export class JevProvider implements DecisionProvider {
  ...
  async evaluateTrace(input: TraceTriageInput, signal: AbortSignal): Promise<TraceLabels> {
    const questions = Object.fromEntries((Object.keys(traceQuestions) as TraceQuestionId[]).map(id => [id,{
      type:'choice',
    ...
    const body=z.object({model:z.literal(this.model),answers:z.record(z.string(),answer),usage:z.object({input_tokens:z.number().int().nonnegative(),output_tokens:z.number().int().nonnegative()})}).parse(JSON.parse(Buffer.concat(chunks).toString('utf8')));
    if(Object.keys(body.answers).length!==Object.keys(traceQuestions).length) throw new SafeError('INVALID_PROVIDER_RESPONSE');
