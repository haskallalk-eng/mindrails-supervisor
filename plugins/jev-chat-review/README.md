...
This Codex plugin has two separate features. The local monitor tracks open Codex chats across sessions and exposes an `open_codex_chats` overview in the Jev tools. It reports current or completed turn duration and exact repeated tool actions/results. It does not guess model fit from prompt length or tool count. Monitoring stores no prompts, tool inputs or outputs, and makes no AI/API calls. Separately, the optional automatic Jev review reads the completed chat transcript and sends a bounded trace to the configured Vercel AI Gateway Jev route.
...
At the end of a Codex turn, the local Stop hook surfaces relevant warnings. The open-chat overview is also available as an on-demand MCP tool rather than a persistent icon or always-visible panel. Ask Jev to “show open chats” to see current sessions and their observations. Exact repeated calls are evidence of a possible loop, not proof of one; Jev recommends changing the approach before changing models. It never infers that a small model is sufficient just because a prompt was short or used few tools. Duration warnings can only be surfaced when a hook event runs or when the overview is requested; the plugin does not run a separate background timer to interrupt a still-running tool. Completed long turns are labeled as past work, not as currently running.
