...
The open-chat overview is an on-demand MCP tool rather than a persistent icon or always-visible panel; Codex does not currently expose that kind of plugin surface. Ask Jev to “show open chats” to see current sessions and their suggestions. The overview is advisory: model recommendations are simple local heuristics and never switch a model automatically. A “stronger model” hint indicates repeated identical tool calls, not a measured comparison of model quality.
