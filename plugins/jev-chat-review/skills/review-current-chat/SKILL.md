...
When `recoveryAdvice` is present, report its title and, if supplied, offer `suggestedPrompt` as a ready-to-use follow-up. It is a fixed application template selected from Jev's progress, blocker and next-step judgments, not prose written by Jev. Do not dispatch it automatically. For `action: none`, including uncertain or conflicting recovery judgments, omit the recovery hint. `CONTINUE_BASELINE` preserves the host's ordinary behavior and does not certify completion or restart stopped work. Clear failure and permission warnings still apply. A model recommendation with `decisionBasis: baseline_fallback` keeps the current model; `quality_fallback` prefers a stronger model despite uncertainty under the user's quality-first policy. Describe that as a policy preference, not a confident model diagnosis. Do not switch models automatically. Raw confidence and option probabilities do not establish that the diagnosis is correct.
