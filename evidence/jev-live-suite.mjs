...
const cases=[
  ...
  {id:'en-contradicted-by-evidence',expected:'continue',evidenceMode:'fact-check',task:'Report whether all tests passed.',requirements:[['status','Report the test status consistently with evidence.']],result:'All tests passed.',evidence:'Test log: 11 passed, 1 failed.'},
...
for(const fixture of cases){
  ...
  const result=await supervisor.check({task:fixture.task,currentResult:fixture.result,evidenceMode:fixture.evidenceMode ?? 'artifact',requirements:fixture.requirements.map(([id,description])=>({id,description})),evidence:fixture.evidence});
