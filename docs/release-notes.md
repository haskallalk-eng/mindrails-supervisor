...
Free software and offline demo. Jev inference is a separate service and may cost money. The native `jev-1.13.0` route was not exercised because new TypeSafe dashboard registrations were unavailable. The Vercel `typesafe-ai/jev` compatibility route was exercised once on twelve frozen synthetic cases: all twelve returned `continue`, scoring 7/12, identical to an always-continue baseline. Jev mode is experimental and this configuration is not recommended as an automated stop gate. Actual MCP stdio integration is tested with the official client. Other hosts are not advertised as verified.
