...
The `plugins/jev-chat-review` package also monitors open Codex chats locally. The local Stop hook surfaces relevant warnings at the end of a turn; ask Jev to open the `open_codex_chats` overview for elapsed time and repeated identical tool-call warnings across sessions. It stores derived signals only, sends no chat content and makes no AI/API calls; it cannot appear as a persistent Codex icon or always-on panel. It has no timer that interrupts a still-running tool. Model-fit guesses based only on prompt length or tool count are intentionally avoided. Separately, the local Stop hook can review available transcripts and recorded tool activity after each completed agent turn with Jev. That optional feature requires `AI_GATEWAY_API_KEY`, sends conversation data to Vercel AI Gateway, and may incur charges. Local defaults cap use at 20 reviews and 350,000 trace bytes per UTC day, with duplicate invocations skipped. These limits reduce usage but do not guarantee a fixed dollar maximum. A Jev request can delay the end of a turn by up to eight seconds. Results are advisory; Jev never continues or closes work, creates issues, or takes actions. Set `MINDRAILS_JEV_AUTO_REVIEW=0` or disable the plugin to stop reviews.
